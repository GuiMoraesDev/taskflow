...
| Hooks | `guard-bash`, `guard-edit`, `progress-reminder` | Ask before `--no-verify`, `git add -A`, bare `npx`-style gate invocations, auto-branching, force pushes and other git force flags, test-runner-config edits and dependency overrides; remind to close the task row after a commit |
